...
class Constraints:
    ...
    def check_time_window(self, route) -> bool:
        ...
        # 从配送中心开始
        current_time = max(self.E[0], current_time)  # 等待配送中心的准备时间
        current_time += self.s[self.instance.location_id_to_index[self.instance.O.id]]  # 考虑配送中心的服务时间
        ...
        for i in range(1, len(route)):
            ...
            print(f"Current time at node {next_node}: {current_time}, ReadyTime: {ready_time}, DueDate: {due_date}")

            if current_time < ready_time:
                # 当前时间早于时间窗的开始时间，等待直到时间窗开始
                current_time = ready_time
                print(f"Waiting at node {next_node} until {ready_time}")

            # 检查是否违反时间窗约束
            if current_time > due_date:
                print(f"Time window violated at node {next_node}. Current time: {current_time}, DueDate: {due_date}")
                return False
    ...
    def check_battery(self, route) -> bool:
        ...
        for i in range(len(route) - 1):
            ...
            if remaining_battery < battery_usage:
                print(f"Battery constraint violated between {current_node} and {next_node}.")
                return False

            remaining_battery -= battery_usage
            print(f"Remaining battery after node {current_node}: {remaining_battery}")

        # 如果未经过充电站或经过多个充电站并试图充电多次，视为违反约束
        if not charged_once or sum(1 for node in route if node in self.R) > 1:
            print("Battery constraint violated: No charging or multiple charges detected.")
            return False
    ...
    def node_visit_constraints(self) -> List:
        ...
        for k in self.K:
            constraints.append(
                sum(self.instance.x_ijk[self.instance.location_id_to_index[self.instance.O.id], j, k] for j in
                    self.C + self.R) ==
            ...
            if route[i] in self.R:
                ...
                if next_station_index:
                    ...
                    if self.B_star - required_battery_to_next_station >= self.soc_min * self.B_star:
                        print(f"Skipping charging at station {route[i]}")
                        continue
                # 否则继续在当前站点充电
                remaining_battery = self.calculate_partial_charge(route[i])
                print(f"Charging at station {route[i]}, remaining battery: {remaining_battery}")
