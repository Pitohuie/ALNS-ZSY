...
def insert_charging_station(instance, current_route, remaining_battery, vehicle_type):
    ...
    for i, customer in enumerate(current_route):
        for charging_station_index in instance.charging_station_indices:
            new_route = current_route[:i + 1] + [charging_station_index] + current_route[i + 1:]

            required_battery = instance.calculate_battery_usage(new_route[i + 1:])
            charging_needed = max(0, required_battery - remaining_battery)

            total_cost = calculate_total_cost(instance, current_route, charging_station_index, vehicle_type) + \
            ...
            if total_cost < best_cost:
                ...
                best_station = charging_station_index
...
def greedy_initial_solution_with_partial_charging(instance, vehicle_type, customers):
    ...
    while unassigned_customers:
        current_route = [instance.depot_index]  # 从配送中心出发
        remaining_capacity = instance.Q_e if vehicle_type == 'electric' else instance.Q_f
        remaining_battery = instance.B_star if vehicle_type == 'electric' else float('inf')
        ...
        while unassigned_customers:
            ...
            for customer in unassigned_customers:
                adjusted_customer_index = customer - 22  # 调整客户索引

                if instance.q_i[adjusted_customer_index] <= remaining_capacity:
                    temp_route = current_route + [customer]
                    temp_cost = calculate_total_cost(instance, current_route, customer, vehicle_type)

                    if temp_cost < best_cost and is_feasible(instance, constraints_checker, temp_route, customer, vehicle_type):
                        best_cost = temp_cost
            ...
            adjusted_customer_index = best_customer - 22  # 调整客户索引
            ...
            if vehicle_type == 'electric':
                ...
                if remaining_battery < instance.minimum_battery_threshold:
                    ...
                    current_route = [instance.depot_index]
