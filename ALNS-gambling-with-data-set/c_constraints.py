from typing import List
from a_read_instance import read_solomon_instance
from b_CCMFEVRP_PRTW_instance import CustomEVRPInstance, Location

class Constraints:
    def __init__(self, instance: CustomEVRPInstance):
        self.instance = instance

        self.C = [instance.location_id_to_index[loc.id] for loc in instance.locations if loc.type == 'c']
        self.R = [instance.location_id_to_index[loc.id] for loc in instance.locations if loc.type == 'f']
        self.N = list(range(instance.N))
        self.K = list(range(instance.K_e + instance.K_f))
        self.K_e = list(range(instance.K_e))
        self.K_f = list(range(instance.K_e, instance.K_e + instance.K_f))
        self.M = instance.M
        self.Q_e = instance.Q_e
        self.Q_f = instance.Q_f
        self.q = instance.q_i
        self.t_e = instance.t_ijk_e
        self.t_f = instance.t_ijk_f
        self.s = instance.q_i
        self.w = instance.w_ik
        self.E = instance.E_i
        self.L = instance.L_i
        self.B_star = instance.B_star
        self.L_ijk_e = instance.L_ijk_e
        self.soc_min = instance.soc_min
        self.e = instance.e

    def check_node_visit(self, route, vehicle_type: str) -> bool:
        # 确保路径从配送中心开始并回到配送中心
        if route[0] != self.instance.location_id_to_index[self.instance.O.id] or route[-1] != self.instance.O_prime:
            return False

        visited = set(route)
        if len(visited) != len(route):
            return False  # 检查路径中是否有重复访问

        for customer in self.C:
            if customer not in visited:
                return False  # 确保所有客户节点都被访问

        if vehicle_type == 'electric':
            # 确保电动车最多只访问一个充电站
            charging_station_visits = sum(1 for node in route if node in self.R)
            if charging_station_visits > 1:
                return False

        return True

    def check_load_balance(self, route, vehicle_type: str) -> bool:
        total_demand = 0
        for node in route:
            if node in self.C:
                total_demand += self.q[node]
            if vehicle_type == 'electric' and total_demand > self.Q_e:
                return False
            elif vehicle_type == 'fuel' and total_demand > self.Q_f:
                return False
        return True

    def check_time_window(self, route, vehicle_type: str) -> bool:
        current_time = 0
        for i in range(len(route) - 1):
            current_time += self.t_e[route[i], route[i + 1]] + self.s[route[i]]
            if current_time > self.L[route[i + 1]]:
                return False  # 违反时间窗约束
            if current_time < self.E[route[i + 1]]:
                current_time = self.E[route[i + 1]]  # 等待直到时间窗开始
        return True

    def check_battery(self, route) -> bool:
        remaining_battery = self.B_star
        charged = False  # 用于记录是否充电

        for i in range(len(route) - 1):
            # 判断是否为充电站节点
            if route[i] in self.R and not charged:
                remaining_battery = self.B_star  # 假设在充电站充满电
                charged = True  # 标记为已充电

            battery_usage = self.L_ijk_e[route[i], route[i + 1]]
            if remaining_battery < battery_usage:
                return False  # 电池电量不足，违反约束

            remaining_battery -= battery_usage

            # 检查部分充电策略，如果有部分充电情况
            if route[i] in self.R and charged:
                remaining_battery += self.calculate_partial_charge(route[i])
                if remaining_battery > self.B_star:
                    remaining_battery = self.B_star

        return remaining_battery >= self.B_star * self.soc_min

    def calculate_partial_charge(self, station_id):
        # 改进部分充电计算，根据充电速率和等待时间
        waiting_time = self.instance.w_ik[station_id, 0]
        charge_rate = self.e  # 假设 e 是充电速率

        partial_charge = charge_rate * waiting_time
        return partial_charge

    def node_visit_constraints(self) -> List:
        constraints = []

        for k in self.K:
            constraints.append(
                sum(self.instance.x_ijk[self.instance.location_id_to_index[self.instance.O.id], j, k] for j in self.C + self.R) ==
                sum(self.instance.x_ijk[j, self.instance.O_prime, k] for j in self.C + self.R)
            )

        for i in self.C:
            constraints.append(
                sum(self.instance.x_ijk[i, j, k] for j in self.N if j != i for k in self.K) == 1
            )

        for j in self.C:
            for k in self.K:
                constraints.append(
                    sum(self.instance.x_ijk[i, j, k] for i in self.N if i != j) ==
                    sum(self.instance.x_ijk[j, i, k] for i in self.N if i != j)
                )

        for k in self.K_e:
            constraints.append(
                sum(self.instance.x_ijk[i, j, k] for i in self.N for j in self.R) <= 1
            )

        return constraints

    def load_balance_constraints(self) -> List:
        constraints = []

        E = [(i, j) for i in self.N for j in self.N if i != j]

        for (i, j) in E:
            for k in self.K_e:
                constraints.append(self.instance.u_ijk[i, j, k] <= self.Q_e)
            for k in self.K_f:
                constraints.append(self.instance.u_ijk[i, j, k] <= self.Q_f)

        for j in self.C:
            for k in self.K:
                constraints.append(
                    sum(self.instance.u_ijk[i, j, k] for i in self.N if i != j) -
                    sum(self.instance.u_ijk[j, i, k] for i in self.N if i != j) +
                    self.M * (1 - sum(self.instance.x_ijk[i, j, k] for i in self.N if i != j)) >= self.q[j]
                )

        for k in self.K:
            constraints.append(
                sum(self.instance.u_ijk[i, self.instance.O_prime, k] for i in self.C + self.R) == 0
            )

        return constraints

    def time_constraints(self) -> List:
        constraints = []

        for k in self.K:
            # 确保车辆离开配送中心的时间不早于开放时间
            constraints.append(
                self.instance.b_ik[self.instance.location_id_to_index[self.instance.O.id], k] >= self.E[0])

        for j in self.C:
            for k in self.K:
                constraints.append(
                    self.instance.b_ik[self.instance.location_id_to_index[self.instance.O.id], k] +
                    self.t_e[self.instance.location_id_to_index[self.instance.O.id], j] *
                    self.instance.x_ijk[self.instance.location_id_to_index[self.instance.O.id], j, k] -
                    self.M * (1 - self.instance.x_ijk[self.instance.location_id_to_index[self.instance.O.id], j, k]) <=
                    self.instance.a_ik[j, k]
                )

        # 添加从路径中返回配送中心的时间约束

        for i in self.C:
            for j in self.N:
                if i != j:
                    for k in self.K:
                        constraints.append(
                            self.instance.a_ik[i, k] + self.t_e[i, j] * self.instance.x_ijk[i, j, k] + self.s[i] + self.w[i, k] - self.M * (1 - self.instance.x_ijk[i, j, k]) <= self.instance.a_ik[j, k]
                        )

        for i in self.R:
            for j in self.N:
                if i != j:
                    for k in self.K_e:
                        constraints.append(
                            self.instance.a_ik[i, k] + self.t_e[i, j] * self.instance.x_ijk[i, j, k] + self.s[i] + self.w[i, k] - self.M * (1 - self.instance.x_ijk[i, j, k]) <= self.instance.a_ik[j, k]
                        )

        for i in self.N:
            for k in self.K:
                constraints.append(
                    self.instance.b_ik[i, k] +
                    self.t_e[i, self.instance.O_prime] *
                    self.instance.x_ijk[i, self.instance.O_prime, k] -
                    self.M * (1 - self.instance.x_ijk[i, self.instance.O_prime, k]) <=
                    self.L[0]
                )

        for i in self.C:
            for k in self.K:
                constraints.append(
                    self.instance.b_ik[i, k] == self.instance.a_ik[i, k] + self.s[i] + self.w[i, k]
                )

        for i in self.R:
            for k in self.K_e:
                constraints.append(
                    self.instance.b_ik[i, k] == self.instance.a_ik[i, k] + self.M
                )

        for i in self.C:
            for k in self.K:
                constraints.append(
                    self.instance.a_ik[i, k] >= self.E[i]
                )
                constraints.append(
                    self.instance.a_ik[i, k] + self.w[i, k] <= self.L[i]
                )
                constraints.append(
                    self.instance.w_ik[i, k] >= self.E[i] - self.instance.a_ik[i, k]
                )

        return constraints

    def validate_route(self, route, vehicle_type: str):
        """
        验证路径是否符合电动车或燃油车的约束条件
        """
        load_carried = sum(self.q[loc_id] for loc_id in route if self.instance.locations[loc_id].type == 'c')
        print(f"Validating route_in: {route} for vehicle type: {vehicle_type}")

        # 检查节点访问约束
        if not self.check_node_visit(route, vehicle_type):
            print("Node visit constraint violated.")
            raise ValueError("Node visit constraint violated.")

        # 检查负载平衡约束
        if not self.check_load_balance(route, vehicle_type):
            print("Load balance constraint violated.")
            raise ValueError("Load balance constraint violated.")

        # 检查时间窗约束
        if not self.check_time_window(route, vehicle_type):
            print("Time window constraint violated.")
            raise ValueError("Time window constraint violated.")

        # 根据车辆类型进行额外检查
        if vehicle_type == 'electric':
            total_energy = self.instance.calculate_energy_consumption(route)
            if total_energy > self.instance.B_star:
                print("Energy consumption exceeds battery capacity.")
                raise ValueError("Energy consumption exceeds battery capacity.")
            if load_carried > self.Q_e:
                print("Load exceeds electric vehicle capacity.")
                raise ValueError("Load exceeds electric vehicle capacity.")
            if not self.check_battery(route):
                print("Battery constraint violated.")
                raise ValueError("Battery constraint violated.")
        elif vehicle_type == 'fuel':
            total_fuel = self.instance.calculate_fuel_consumption(route)
            if load_carried > self.Q_f:
                print("Load exceeds fuel vehicle capacity.")
                raise ValueError("Load exceeds fuel vehicle capacity.")
        else:
            raise ValueError("Unknown vehicle type.")

        print("All constraints passed for route_in:", route)

    def battery_constraints(self) -> List:
        constraints = []

        for i in self.N:
            for j in self.N:
                if i != j:
                    for k in self.K_e:
                        constraints.append(
                            self.instance.B_ik1[i, k] - self.L_ijk_e[i, j] * self.instance.x_ijk[i, j, k] + self.B_star * (1 - self.instance.x_ijk[i, j, k]) >= self.instance.B_ik1[j, k]
                        )

        for k in self.K_e:
            constraints.append(
                self.instance.B_ik1[self.instance.location_id_to_index[self.instance.O.id], k] == self.B_star
            )

        for i in self.N:
            for k in self.K_e:
                constraints.append(
                    self.instance.B_ik1[i, k] >= self.B_star * self.soc_min
                )

        for r in self.R:
            for k in self.K_e:
                constraints.append(
                    self.instance.B_ik1[r, k] == self.B_star
                )

        return constraints

    def variable_constraints(self) -> List:
        constraints = []

        for (i, j) in [(i, j) for i in self.N for j in self.N if i != j]:
            for k in self.K:
                constraints.append(self.instance.u_ijk[i, j, k] >= 0)
                if k in self.K_e:
                    constraints.append(self.instance.f_ijk[i, j, k] >= 0)
                constraints.append(self.instance.x_ijk[i, j, k] == 0 or self.instance.x_ijk[i, j, k] == 1)

        return constraints

if __name__ == "__main__":
    import os

    # 调试当前工作目录和文件列表
    print("Current working directory:", os.getcwd())
    print("Files in current directory:", os.listdir())

    # 使用绝对路径
    file_path = "D:/2024/ZSY-ALNS/pythonProject1/evrptw_instances/c101_21.txt"  # 替换为你的文件绝对路径

    try:
        locations_data, vehicles_data = read_solomon_instance(file_path)
        locations = [Location(**loc) for loc in locations_data]
        instance = CustomEVRPInstance(locations, vehicles_data)
        constraints_instance = Constraints(instance)

        # 调用并测试约束检查
        route = [0, 1, 2, 3, 4, 5]  # 示例路径

        if constraints_instance.check_node_visit(route, 'electric'):
            print("Node visit constraint passed for electric vehicle.")
        else:
            print("Node visit constraint failed for electric vehicle.")

        if constraints_instance.check_load_balance(route, 'electric'):
            print("Load balance constraint passed for electric vehicle.")
        else:
            print("Load balance constraint failed for electric vehicle.")

        if constraints_instance.check_time_window(route, 'electric'):
            print("Time window constraint passed for electric vehicle.")
        else:
            print("Time window constraint failed for electric vehicle.")

        if constraints_instance.check_battery(route):
            print("Battery constraint passed for electric vehicle.")
        else:
            print("Battery constraint failed for electric vehicle.")

    except FileNotFoundError as e:
        print(f"Error: {e}")
