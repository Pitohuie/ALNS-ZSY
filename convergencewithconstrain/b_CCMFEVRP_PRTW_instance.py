...
class CustomEVRPInstance:
    ...
    def __post_init__(self):
        ...
        # 校验 q_i, E_i, L_i 的大小
        for array_name, array in zip(['q_i', 'E_i', 'L_i'], [self.q_i, self.E_i, self.L_i]):
            if len(array) != len(self.locations):
                raise ValueError(f"Mismatch between {array_name} size ({len(array)}) and total locations ({len(self.locations)}).")
