...
class CustomEVRPInstance:
    ...
    B_star: float = 200  # 电动车和燃油车的默认电池容量
    ...
    def __post_init__(self):
        ...
        self.B_star = self.vehicles.get('battery_capacity', 50)
        ...
        # 确保这些变量的大小与所有位置数量一致
        if len(self.q_i) != len(self.locations):
            raise ValueError(
                f"Mismatch between q_i size ({len(self.q_i)}) and total locations ({len(self.locations)}).")

        if len(self.E_i) != len(self.locations):
            raise ValueError(
                f"Mismatch between E_i size ({len(self.E_i)}) and total locations ({len(self.locations)}).")

        if len(self.L_i) != len(self.locations):
            raise ValueError(
                f"Mismatch between L_i size ({len(self.L_i)}) and total locations ({len(self.locations)}).")
        ...
        if len(self.q_i) != len(self.locations):
            raise ValueError(
                f"Mismatch between q_i size ({len(self.q_i)}) and number of locations ({len(self.locations)}).")

        if len(self.E_i) != len(self.locations):
            raise ValueError(
                f"Mismatch between E_i size ({len(self.E_i)}) and number of locations ({len(self.locations)}).")

        if len(self.L_i) != len(self.locations):
            raise ValueError(
                f"Mismatch between L_i size ({len(self.L_i)}) and number of locations ({len(self.locations)}).")
    ...
    def calculate_time_window_penalty(self, route_in, vehicle_type: str) -> float:
        ...
        for i in range(1, len(route_in)):
            ...
            ready_time = self.E_i[end - 22]
            due_date = self.L_i[end - 22]
