...
class DestructionOperators:
    ...
    def time_satisfaction_similarity_removal(self, curr_solution, *args, **kwargs):
        customers = self.get_customers_from_solution(curr_solution)
        if len(customers) < self.num_customers_to_remove:
            print("Not enough customers available for removal. Skipping operation.")
            return curr_solution

        reference_customer = self.rnd_state.choice(customers)
        satisfaction_gap = {}

        # Calculate time satisfaction for the reference customer
        reference_satisfaction = curr_solution.instance.calculate_time_satisfaction(reference_customer)

        for customer in customers:
            if customer != reference_customer:
                customer_satisfaction = curr_solution.instance.calculate_time_satisfaction(customer)
