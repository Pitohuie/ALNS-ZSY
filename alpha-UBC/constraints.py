...
def calculate_insertion_cost(instance, route, pos, customer):
    # 计算插入客户的成本
    prev_customer = route[pos - 1]
    next_customer = route[pos] if pos < len(route) else instance.O_prime
...
def repair_solution(instance, route):
    for i in range(1, len(route) - 1):
        if route[i] >= instance.n:
            route.pop(i)
            if verify_time_windows(instance, route):
                return route
        elif route[i] < instance.n:
            removed_customer = route.pop(i)
            if verify_time_windows(instance, route):
                return route
            else:
                route.insert(i, removed_customer)
    return route
